...
#从输入的描述符（通常是一些高维向量）中生成一个置信度分数。这个分数通常表示为概率（介于0和1之间），用于某些任务中判断描述符的可信度或重要性。
...
class LightGlue(nn.Module):
    default_conf = {
        ...
        "depth_confidence": 0.95,  # early sptopping, disable with -1
        "width_confidence": 0.99,  # point runing, disable with -1
        "filter_threshold": 0.01,  # match threshold 0.1
        "weights": None,
    }
