...
from dh_request import send_login_req,send_get_device_tree
...
class TcpClient():
    ...
    def handle_send_messages(self):
        ...
        """
        print("执行发送逻辑")
        groupData = get_previous_minute_data(client)
        print("定时发送消息",groupData)

        res_bytes = ("UD" + self.config.get('server', 'imei')  + "MONIX" + "+" +str(groupData)).encode('utf-8')

        # print(f"定时推送消息 deform1, 长度: {len(res_bytes)}",res_bytes)
